...
//!音频监听
#define TT_MEDIAPLAYER_AM_NUM		16	// 修改为8声道

#define TT_ERROR_CreateIndexFailed  1001

////！新版本使用 TTVS::TP_VIDEO_STANDARD
//enum	TT_MEDIAPLAYER_VIDEO_STANDARD	// 桌面视频制式标准枚举定义
//{
...
//    TP_MP_VIDEOSTANDARD_NTSC_2997_480		= 0x00400000,		//NTSC size:720*480  f/s : 29.97  //先留着，防止以后特殊情况再改成480
//    TP_MP_VIDEOSTANDARD_NTSC_30_480		    = 0x00800000,		//NTSC size:720*480 f/s : 30 //先留着，防止以后特殊情况再改成480
//
...
enum TTPixelFormat 
{
	...
	TT_PIX_FMT_SPLIT_ALPHA	= 0x00040000, ///是否具有独立的alpha buffer
...
enum TT_MEDIAPLAYER_HDSD_MODE
{
	...
	TT_MEDIAPLAYER_HDSD_CutHorz,//!水平适配
	TT_MEDIAPLAYER_HDSD_EstopVert,//!垂直适配
};
//!奇偶场显示
enum TT_MEDIAPLAYER_DISPLAY_MODE
{
	TT_MEDIAPLAYER_DSP_NO	= 0, //!不显示
...
{
	BOOL						bEnableOutput;	//!是否输出到监视器
	BOOL						bEnableOutputAudio;//!是否从Io卡输出音频,前提是bEnableOutput有效
	TT_MEDIAPLAYER_DISPLAY_MODE	eIOOutputMode;//!IO卡是输出方式（在seek的时候有效）
	GUID						guidVideoOutEngine;//!I0卡输出引擎
	TCHAR						szOutEngineName[64];

	BOOL						bEnableAudioCard;//!是否从声卡输出音频
	int							iAudiSeekFramesNext;//!音频seek后几帧
	TT_MEDIAPLAYER_DISPLAY_MODE	eDisplayMode;//!预监的方式
	BOOL						bMultipleZoom;//!预监窗口根据视频幅面倍数缩放显示
	TT_MEDIAPLAYER_HDSD_MODE	eHDSDMode;

	BOOL						b169SafeBox; //!安全框
	BOOL						b43SafeBox;
	int							iSafeRectH;
	int							iSafeRectV;
	BOOL						bDropFrame;	//视音频不实时，是否丢帧显示
...
typedef struct stu_TTMediaPlayerIOCard
{
	...
	TCHAR							szOutEngineName[64];
	stuTTVideoStandard				tpVideoStandard;//!支持的制式输出
...
{
	BOOL  bPeekValue;						 // TRUE 峰值 FALSE VU 值
	WORD  wBitsPerSample; 	
	int   iAudioTrackValue[TT_MEDIAPLAYER_AUDIO_NUM];          // 轨道音量
	int   iAudioOutValue[TT_MEDIAPLAYER_AUDIO_NUM];			// 输出通道音量
...
//!Editmax7,Nova ,Enet5 , EM2 等等系统或者数据库记录的媒体文件格式信息
typedef struct stu_TTMediaPlayerMediaFormatForE7
{
	BYTE					bFormatType;				//1：视频格式，2：音频格式，3：键格式，4：图片格式
	GUID					guidFileType;			//文件格式的GUID
	long					lDataRate;			//码率
	long					lFOURCC;				//媒体类型
	long					lMediaType;			//媒体文件类型
	long					lSubMediaType;		//媒体文件子类型
	long					lAudioChannels;		//音频通道数
	long					lAudioSamplesRate;	//音频采样率
	long					lAudioBitsperSample;	//量化位数
	long					lVideoImgWidth;		//素材画面宽
	long					lVideoImgHeight;		//素材画面高
	stuTTVideoStandard		tpVideoStandard;		//制式
	long					lVPixFmt;				//像素格式
	long					lVBitsPerPixel;		//每像素位数
	double					dblVFrameRate;		//帧率
	long					lVCompression;		//数据压缩方式
	long					lVScanMode;			//扫描模式
	long					lVGOPSize;			//GOP长度
	long					lVBFrameCount;		//B帧个数
	long					lVPFrameCount;		//P帧个数
	//构造函数
...
typedef struct stu_TTMediaPlayerFileItem
{
	...
	TT_MEDIAPLAYER_CLASS_TYPE	dwChannel;//!确定是否视频或者音频（如果是音频那么有声道的选择）
	__int64						iFileIn;
	__int64						iFileOut;
	__int64						iTrackIn;
	__int64						iTrackOut;
	...
	lpTTMediaPlayerFileFormat	lpFileFormat; //!如果为空，TTMediaPlayer自动校验
	GUID						guidFilePlugin;// 选择文件Cutlist guid,如果没有特殊要求一般为GUID_NULL;
	GUID						guidFile; //!数据惟一的标识别
	HTTMPHANDLE				    hFileData; //数据句柄，为CG预留
	DWORD_PTR					dwParam;
...
typedef struct stu_TTMediaPlayerSequence
{
	...
	TTMediaPlayerTrack			stuVideoTrack;	    //!视频轨道
	TTMediaPlayerTrack			stuAudioTrack[TT_MEDIAPLAYER_AUDIO_NUM]; //!音频轨道
...
{	
	...
	//!获取Sequence的有效入出点，Sequence的长度为lOutPoint;Sequence的有效长度为(lOutPoint - lInPoint)
	...
	int			(* TT_MPI_GetCurrentPosAndVU)(HTTMPHANDLE hSequence,__int64 & iPos,TTMediaPlayerVUData ** pVUDat);//!外部不能del该指针
