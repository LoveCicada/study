...
enum TTESCANMODE
{
	...
	ttScanMode_Field        = 1,   //场编辑
	ttScanMode_Progressive  = 2    //帧编辑
};

enum TTEDEINTERLACETYPE 
{
	ttDeinterlaceType_Invalid        = 0,
	ttDeinterlaceType_TopFirst       = 1,   //顶场优先
	ttDeinterlaceType_BottomFirst    = 2    //底场优先
};
...
private:
	...
	DWORD				m_dwEditWidth;                   //编辑高度
	DWORD				m_dwEditHeight;                  //编辑宽度
	DWORD				m_dwFrameRateFrequency;			 //帧速
	DWORD				m_dwFrameRateScale;				 //帧率倍数
	short				m_sWidthAspectRatio;             //幅面宽度比
	short				m_sHeightAspectRatio;            //幅面高度比
	short				m_sWidthPixRatio;				 //像素宽度比
	short				m_sHeightPixRatio;				 //像素高度比
	short				m_sViewNum;                      //3D双目/单窗口
	short				m_sBitCount;                     //位宽
	TTESCANMODE			m_eScanMode;					 //帧,场编辑
	TTEDEINTERLACETYPE	m_eDeinterlaceType;				 //顶场/底场优先
	TCHAR				m_chColorsSpace[MAX_COLORSPACE]; //颜色空间
...
public:
	...
	long GetHashKey() const
	{
		...
		//2020-06-04,zhangfan,如果不初始化为0，里面会有一些错误的值，影响lKey的计算。
		TCHAR pColorsSpace[MAX_COLORSPACE] = {0};
		_tcscpy_s(pColorsSpace, m_chColorsSpace);
		//将高清且没有颜色空间的素材颜色空间初始化为Rec.709
