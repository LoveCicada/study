...
namespace subprocess
{
    ...
    public:
        popen(const std::string &cmd, std::vector<std::string> argv)
            : in_filebuf(nullptr), out_filebuf(nullptr), err_filebuf(nullptr),
              in_stream(nullptr), out_stream(nullptr), err_stream(nullptr)
        {
            if (pipe(in_pipe) == -1 ||
                pipe(out_pipe) == -1 ||
                pipe(err_pipe) == -1)
        ...
        //! results
        bool getResults(std::vector<std::string> &vs)
        {
            std::string str;
            while (std::getline(stdout(), str))
            {
                vs.push_back(str);
            }
        ...
        void printResults(std::vector<std::string> &vs)
        {
